...
class GridWorld:
    ...
    def run_episodes(self):
        ...
        while reach_threshold == 0:
            ...
            while not is_terminal:
                ...
                if state == self.goal:
                    ...
                    if random.random() < EPSILON:
                        ...
                        action = max(self.ACTIONS, key=lambda action: self.q_values[state, action])

                    dice = random.random()

                    if dice > 1 - NOISE:
                        noise_actions = [element for element in self.ACTIONS if element != action]
                        get_index = random.randrange(len(noise_actions))
                        action = noise_actions[get_index]

                    row = state[0] + action[0]
                    col = state[1] + action[1]
                    next_state = (row if 0 <= row < self.size else state[0],
                                  col if 0 <= col < self.size else state[1])
                    if self.grid[next_state[0]][next_state[1]] == np.inf:
                        next_state = state
                    self.curState = next_state
                    value = max(self.q_values[next_state, action] for action in self.ACTIONS)

                reward = self.grid[state[0]][state[1]] - 1
                self.cnt[state, action] += 1
                difference = (reward + DISCOUNT * value -
                              self.q_values[state, action])
                self.q_values[state, action] += ALPHA * difference
                count += 1

            episodes = episodes + 1
            max_diff = 0
            for key, value in self.q_values.items():
                diff = value - qnext[key];
                if (diff > max_diff):
                    max_diff = diff
            if (max_diff < 0.0001):
                reach_threshold = 1
            score += reward
            self.curState = self.start
        print(f"the agent took : {episodes} episodes to learn an optimal policy, " +
                "the convergence condition is changes between episodes are below " +
                "a small threshold, e.g. 0.0001")
        policy = {}
        states = [(i, j) for i in range(self.size)
                for j in range(self.size) if self.grid[i][j] != np.inf]
        for state in states:
            if state == self.goal:
                continue
            best = max(self.ACTIONS, key=lambda action: self.q_values[state, action])
            policy[state] = best
...
def main():
    ...
    print(f'the value (utility) of the starting point: {world.q_values[(world.start,world.optimal_policy[world.start])]}')
